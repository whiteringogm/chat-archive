...
function renderSearchBulk() {
  const box = $("searchBulk"),
    chosen = all.filter((s) => bulkSelected.has(s.id));
  if (!chosen.length) {
    box.innerHTML = "";
    return;
  }
...
function renderList() {
  ...
  $("summary").textContent =
    `${rows.length} / ${all.length} セッション · ${all.reduce((n, s) => n + s.messages.filter((m) => !m.hidden).length, 0)} 発言`;
  $("sessions").innerHTML =
    rows
      .map((s) => {
...
function personaOptions() {
  return [
    ...new Set([
      ...BUILTIN_PERSONAS,
      ...settings.customPersonas,
      ...all.map((x) => x.persona).filter(Boolean),
    ]),
  ];
}
...
function rebuildFilters() {
  ...
  $("model").innerHTML =
    ...
    [...new Set(all.flatMap((x) => x.models))]
      .sort()
...
function renderMemories() {
  ...
  $("viewer").scrollTop = 0;
  $("viewer").innerHTML =
    `<section class="archive-browser"><nav class="breadcrumbs"><button id="backToFolders">フォルダ一覧</button><span>›</span><strong>思い出一覧</strong></nav><div class="browser-heading"><div><p class="browser-kicker">MEMORIES</p><h2>思い出一覧</h2><p class="muted">保存したタイトルを選ぶと、元の会話の該当箇所へ移動します。</p></div><span class="count-badge">${items.length} 件</span></div><div class="memory-list">${items.map(({ session, memory }) => `<div class="memory-entry"><button data-open-memory="${esc(memory.id)}" data-session-id="${esc(session.id)}"><strong>${esc(memory.title)}</strong><small>${esc(replaceText(session.title))} · ${esc(session.folder || "未分類")} · ${memory.messageIds.length}件</small></button><button class="memory-delete" data-delete-memory="${esc(memory.id)}" data-session-id="${esc(session.id)}">削除</button></div>`).join("") || '<div class="empty browser-empty"><div class="moon">✦</div><h2>保存した思い出はまだありません</h2><p>セッション内で発言を選択し、「タイトルをつけて保存」するとここへ並びます。</p></div>'}</div></section>`;
...
function renderFolderBrowser() {
  const folders = folderOptions()
    .map((name) => ({
      name,
      sessions: all.filter((s) => (s.folder || "未分類") === name),
    }))
    .filter(
