...
$("sidebarBackdrop").onclick=closeSidebar;$("sessionMenuBtn").onclick=()=>document.body.classList.contains("session-panel-open")?closeSessionPanel():openSessionPanel();$("sessionPanelBackdrop").onclick=closeSessionPanel;$("exitFocusMode").onclick=()=>setFocusMode(false);
$("homeBtn").onclick = () => {
  showFolders();
...
function defaultPersonaProfiles() {
  return [
    { id: "persona-kage", emoji: "🕯️", emojiAliases: ["🕯"], displayName: "🕯️ かげちゃん", identifier: "影山誠実", aliases: ["かげちゃん", "🕯️かげちゃん", "🕯かげちゃん", "🕯 かげちゃん"], diaryKind: "today", diaryLabel: "今日の記録" },
    { id: "persona-man", emoji: "🌕", emojiAliases: [], displayName: "🌕 まんちゃん", identifier: "温間満月", aliases: ["まんちゃん", "🌕まんちゃん"], diaryKind: "closing", diaryLabel: "締めログ" },
    { id: "persona-hika", emoji: "🔦", emojiAliases: [], displayName: "🔦 ひかちゃん", identifier: "光谷虚実", aliases: ["ひかちゃん", "🔦ひかちゃん"], diaryKind: "daily-diff", diaryLabel: "日次差分" },
  ];
}
const DIARY_KINDS = {
  today: { label: "今日の記録" },
...
function defaultDiaryKindForPersonaId(id) {
  if (id === "persona-man") return "closing";
  if (id === "persona-hika") return "daily-diff";
  if (id === "persona-kage") return "today";
  return id ? "diary-" + id : "today";
}
function normalizeDiaryKind(value, personaId = "") {
  const kind = String(value || "").trim();
  return kind || defaultDiaryKindForPersonaId(personaId);
}
function defaultDiaryLabel(value, personaId = "") {
  if (personaId === "persona-hika" || value === "daily-diff") return "日次差分";
  return DIARY_KINDS[value]?.label || "今日の記録";
}
function diaryKindLabel(value, personaValue = "") {
  const profile = personaValue ? personaProfileFor(personaValue) : null;
  if (profile && profile.diaryKind === value) return profile.diaryLabel;
  const profiles = Array.isArray(settings?.personaProfiles) ? settings.personaProfiles.filter(item => item.diaryKind === value) : [];
  const labels = [...new Set(profiles.map(item => item.diaryLabel).filter(Boolean))];
  return labels.length === 1 ? labels[0] : DIARY_KINDS[value]?.label || String(value || "種別未設定");
}
function personaTextKey(value) {
  return String(value || "").normalize("NFKC").replace(/\uFE0F/g, "").replace(/\s+/g, "").toLowerCase();
}
function personaEmojiKey(value) {
  return String(value || "").normalize("NFKC").replace(/\uFE0F/g, "").replace(/\s+/g, "");
}
function inferredPersonaEmoji(value) {
  const first = String(value || "").trim().split(/\s+/)[0] || "";
  return /[^\p{L}\p{N}\p{P}\p{Z}]/u.test(first) ? first : "";
}
function normalizePersonaProfile(profile, index = 0) {
  ...
  let diaryKind = normalizeDiaryKind(profile?.diaryKind, id);
  if (id === "persona-hika" && diaryKind === "today" && !profile?.diaryLabel) diaryKind = "daily-diff";
  return {
    id,
...
function ensurePersonaProfiles() {
  if (!Array.isArray(settings.personaProfiles)) {
    const legacyCustom = Array.isArray(settings.customPersonas) ? settings.customPersonas : [];
    settings.personaProfiles = defaultPersonaProfiles().concat(legacyCustom
      .filter(name => !BUILTIN_PERSONAS.includes(name) && name !== "未分類")
      .map((name, index) => normalizePersonaProfile({
        id: `persona-custom-${index}-${personaTextKey(name) || Date.now()}`,
        displayName: name,
        identifier: String(name).replace(/^\S+\s*/, "") || name,
        aliases: [name],
      }, index)));
  } else {
    settings.personaProfiles = settings.personaProfiles
      .map(normalizePersonaProfile)
      .filter(profile => profile.displayName && profile.identifier);
  }
  const ids = new Set();
  settings.personaProfiles.forEach((profile, index) => {
    if (ids.has(profile.id)) profile.id = `${profile.id}-${index}`;
    ids.add(profile.id);
  });
  settings.customPersonas = settings.personaProfiles
    .filter(profile => !profile.id.startsWith("persona-kage") && !profile.id.startsWith("persona-man") && !profile.id.startsWith("persona-hika"))
    .map(profile => profile.displayName);
  return settings.personaProfiles;
}
function personaProfiles() {
  if (!Array.isArray(settings.personaProfiles)) ensurePersonaProfiles();
  return settings.personaProfiles;
}
function personaProfileFor(value) {
  const raw = String(value || "");
  const key = personaTextKey(raw);
  if (!key) return null;
  return personaProfiles().find(profile =>
    profile.id === raw ||
    [profile.displayName, profile.identifier, ...(profile.aliases || [])].some(name => personaTextKey(name) === key)
  ) || null;
}
function personaProfileForEmoji(value) {
  const key = personaEmojiKey(value);
  if (!key) return null;
  return personaProfiles().find(profile =>
    [profile.emoji, ...(profile.emojiAliases || [])].some(emoji => personaEmojiKey(emoji) === key)
  ) || null;
}
function personaDisplay(value) {
  if (!value || value === "未分類") return "未分類";
  return personaProfileFor(value)?.displayName || String(value);
}
...
function guessPersona(c, msgs) {
  ...
  if (/まんちゃん|温間満月/.test(hay)) return personaProfileFor("persona-man")?.displayName || "未分類";
  if (/かげちゃん|影山誠実/.test(hay)) return personaProfileFor("persona-kage")?.displayName || "未分類";
  if (/ひかちゃん|光谷虚実/.test(hay)) return personaProfileFor("persona-hika")?.displayName || "未分類";
  return "未分類";
}
...
async function load() {
  try {
    ...
    let folderPersonaMigrated = false;
    all.forEach((s) => {
      s.personaId = personaIdFor(s.personaId || s.persona);
      ...
      if (applyFolderPersonaToSession(s)) folderPersonaMigrated = true;
    });
    if (folderPersonaMigrated) await dbSet(STORE, all);
...
function applyFolderPersonaToSession(s) {
  const folderPersona = folderPersonaForSession(s);
  if (!folderPersona) return false;
  const profile = personaProfileFor(folderPersona);
  const nextPersona = profile?.displayName || personaDisplay(folderPersona);
  const nextPersonaId = profile?.id || personaIdFor(folderPersona);
  const changed = s.persona !== nextPersona || (s.personaId || "") !== nextPersonaId;
  s.persona = nextPersona;
  s.personaId = nextPersonaId;
  return changed;
}
...
function captureReadingPosition() {
  ...
  if (!viewer || !conversation) return null;
  const mobile = matchMedia("(max-width:760px)").matches;
  const viewport = mobile
    ? { top: 0, bottom: window.innerHeight }
    : viewer.getBoundingClientRect();
  const candidates = [...conversation.querySelectorAll(
    ".message .body, .memo-card .markdown-body, .note-entry",
  )].map((element) => ({ element, rect: element.getBoundingClientRect() }));
  const visible = candidates.filter(
    ({ rect }) => rect.bottom > viewport.top && rect.top < viewport.bottom,
  );
  const guide = viewport.top + Math.min(72, (viewport.bottom - viewport.top) * 0.15);
  const anchor =
    visible.find(({ rect }) => rect.top <= guide && rect.bottom > guide) ||
    visible[0] ||
    { element: conversation, rect: conversation.getBoundingClientRect() };
  return {
...
function setFocusMode(enabled) {
  ...
  if (position?.element?.isConnected) {
    holdElementAtViewportTop(() => position.element, position.top);
  } else if (position) {
...
function renderSearchBulk() {
  ...
  $("searchMoveSessions").onclick = async () => {
    ...
    all.forEach((s) => {
      if (!bulkSelected.has(s.id)) return;
      s.folder = destination;
      applyFolderPersonaToSession(s);
    });
...
function renderViewer({updatePanel=true, autoFocusHit=true}={}) {
  ...
  const q=sessionMessageQuery.trim(),model=sessionModelFilter,allShown=visibleMessages(s),directMatches=allShown.filter(m=>(!model||(m.role==="assistant"&&m.model===model))&&(!q||includesAllTerms(m.text, q))),contextIds=new Set(directMatches.map(m=>m.id)),shown=(()=>{if(model&&sessionIncludePrevious)directMatches.forEach(m=>{const i=allShown.findIndex(x=>x.id===m.id);for(let j=i-1;j>=0;j--)if(allShown[j].role==="user"){contextIds.add(allShown[j].id);break}});return allShown.filter(m=>contextIds.has(m.id))})(),
    ...
    folderPersona = folderPersonaForSession(s),
    folderFixed = folderPersona ? personaDisplay(folderPersona) : "",
    selectedPersona = folderFixed || personaDisplay(s.personaId || s.persona);
  $("viewer").innerHTML =
    `<article class="conversation"><div class="title-row"><div><h2>${esc(replaceText(s.title))}</h2><div class="meta">${esc(date(s.time))} · ${esc(s.models.join(", ") || "モデル不明")}</div></div><div class="tools"><button id="addTopMemo">＋ 冒頭にメモ</button><button id="exportMd">Markdown</button><button id="exportJson">JSON</button></div></div><div class="saved-selection-nav"><div class="saved-selection-head"><strong>保存した思い出</strong><span>${s.namedSelections.length}件</span></div>${s.namedSelections.length ? `<div class="saved-selection-list">${s.namedSelections.map((x) => `<button data-memory-id="${esc(x.id)}" class="${x.id === activeNamedSelection ? "active" : ""}">${esc(x.title)}（${x.messageIds.length}件）</button>`).join("")}</div>${active ? '<div class="saved-selection-actions"><button id="editNamedSelection">会話を追加・除去</button><button id="renameNamedSelection">名前を変更</button><button id="deleteNamedSelection" class="memory-remove">この思い出を削除</button></div>' : ""}` : '<p class="muted">発言を選択して「タイトルをつけて保存」すると、ここや思い出一覧から開けます。</p>'}</div><div class="session-fields"><label>ペルソナ<select id="sessionPersona" ${folderFixed ? "disabled" : ""}>${personaOptions()
      .map(
        (x) =>
          `<option ${x === selectedPersona ? "selected" : ""}>${esc(x)}</option>`,
      )
      .join(
        "",
  ...
  $("sessionPersona").onchange = (e) => {
    ...
    s.personaId = personaIdFor(e.target.value);
    save();
    rebuildFilters();
    renderList();
    renderViewer();
  };
  $("sessionFolder").onchange = async (e) => {
...
function personaOptions() {
  return [
    ...new Set([
      ...personaProfiles().map(profile => profile.displayName),
      "未分類",
      ...activeSessions().map((x) => personaDisplay(x.personaId || x.persona)).filter(Boolean),
    ]),
  ];
}
...
function memoryExportPayload(session, memory) {
  ...
  return {
    ...
    sourceSession: {
      ...
      folder: memoryFolder(session, memory),
      persona: effectivePersona(session),
    },
...
renderFolderSessions = function (folder) {
  ...
  if (folder !== "未分類") {
    ...
    $("folderPersonaSetting").onchange = async (e) => {
      if (e.target.value) {
        settings.folderPersonas[folder] = personaIdFor(e.target.value) || e.target.value;
        all.filter(s => !s.trashedAt && (s.folder || "未分類") === folder).forEach(applyFolderPersonaToSession);
      } else {
        delete settings.folderPersonas[folder];
      }
      await save();
      rebuildFilters();
      renderList();
...
personaEditor.className = "persona-hut-dialog";
document.body.append(personaEditor);
function renderPersonaHut() {
  const profiles = personaProfiles();
  $("personaHutList").innerHTML = profiles.map(profile =>
    '<button class="persona-hut-card" type="button" data-persona-profile="' + esc(profile.id) + '"><span><strong>' + esc(profile.displayName) + '</strong><small>識別名：' + esc(profile.identifier) + ' · 別名 ' + profile.aliases.length + '件</small></span><span class="persona-hut-marker">' + (profile.emoji ? '📓' + esc(profile.emoji) : '絵文字未設定') + '<small>' + esc(profile.diaryLabel) + '</small></span></button>'
...
function openPersonaHut() {
  renderPersonaHut();
  if (!personaHut.open) personaHut.showModal();
}
function personaAliasesFrom(text) {
  return [...new Set(String(text || "").split(/[、,，\n]/).map(x => x.trim()).filter(Boolean))];
}
function updatePersonaReferences(profile, previous = {}) {
  const previousValues = [previous.id, previous.displayName, previous.identifier, ...(previous.aliases || [])].filter(Boolean);
  const matchesPrevious = value => value === profile.id || previousValues.some(old => personaTextKey(old) === personaTextKey(value));
  all.forEach(session => {
    if (session.personaId === profile.id || matchesPrevious(session.persona)) {
      session.personaId = profile.id;
      session.persona = profile.displayName;
    }
  });
  Object.keys(settings.folderPersonas || {}).forEach(folder => {
    if (matchesPrevious(settings.folderPersonas[folder])) settings.folderPersonas[folder] = profile.id;
  });
  if (Array.isArray(settings.diaryEntries)) settings.diaryEntries.forEach(entry => {
    if (entry.personaId === profile.id || matchesPrevious(entry.persona)) {
      entry.personaId = profile.id;
      entry.persona = profile.displayName;
    }
  });
  if (Array.isArray(settings.diaryExcludedSources)) settings.diaryExcludedSources.forEach(entry => {
    if (entry.personaId === profile.id || matchesPrevious(entry.persona)) {
      entry.personaId = profile.id;
      entry.persona = profile.displayName;
    }
...
function openPersonaEditor(id = "") {
  ...
  personaEditor.querySelector("#personaSave").onclick = async () => {
    ...
    const diaryLabel = personaEditor.querySelector("#personaEditDiaryLabel").value.trim();
    const diaryKind = normalizeDiaryKind(base.diaryKind, base.id);
    if (!emoji || !diaryLabel || !displayName || !identifier) return alert("日記絵文字・日記の種別名・表示名・識別名を入力してください。");
    ...
    const nameKeys = new Set([displayName, identifier, ...aliases].map(personaTextKey));
    const duplicateName = personaProfiles().find(profile => profile.id !== base.id && [profile.displayName, profile.identifier, ...(profile.aliases || [])].some(name => nameKeys.has(personaTextKey(name))));
    if (duplicateName) return alert(`「${duplicateName.displayName}」と同じ表示名・識別名・別名があります。`);
    const previous = existing ? { ...existing, aliases: [...existing.aliases], emojiAliases: [...existing.emojiAliases] } : {};
    const next = existing || base;
    if (existing && personaEmojiKey(existing.emoji) !== personaEmojiKey(emoji) && existing.emoji) next.emojiAliases = [...new Set([...(existing.emojiAliases || []), existing.emoji])];
    next.emoji = emoji;
    ...
    ensurePersonaProfiles();
    updatePersonaReferences(next, previous);
    await save();
    rebuildFilters();
    renderList();
    if (viewMode === "diaries") dlRender(); else renderViewer();
    personaEditor.close();
    renderPersonaHut();
  };
  personaEditor.querySelector("#personaDelete")?.addEventListener("click", async () => {
    if (!confirm(`「${existing.displayName}」を管理小屋から削除しますか？\n過去データの表示名は残ります。`)) return;
    settings.personaProfiles = personaProfiles().filter(profile => profile.id !== existing.id);
    all.forEach(session => { if (session.personaId === existing.id) session.personaId = ""; });
    Object.keys(settings.folderPersonas || {}).forEach(folder => { if (settings.folderPersonas[folder] === existing.id) settings.folderPersonas[folder] = existing.displayName; });
    if (Array.isArray(settings.diaryEntries)) settings.diaryEntries.forEach(entry => { if (entry.personaId === existing.id) entry.personaId = ""; });
    ensurePersonaProfiles();
    await save();
    rebuildFilters();
    renderList();
...
$("personaHutAdd").onclick = () => openPersonaEditor();
const legacyPersonaManager = $("newPersona")?.closest(".manager");
if (legacyPersonaManager) {
  legacyPersonaManager.style.display = "none";
  const launcher = document.createElement("section");
  launcher.className = "manager persona-hut-launch";
  launcher.innerHTML = '<strong>ペルソナ</strong><p class="muted">初期搭載分も、表示名・識別名・別名・日記絵文字・日記の種別名を編集できます。</p><button id="openPersonaHut" type="button">🛖 ペルソナ管理小屋を開く</button>';
...
const dlLockedKindCss = document.createElement("style");
dlLockedKindCss.textContent = ".dl-fields select:disabled{opacity:.68;background:color-mix(in srgb,var(--card) 72%,#999);color:var(--muted);cursor:not-allowed}";
document.head.append(dlLockedKindCss);
...
function dlNormalizeEntry(entry) {
  const before = JSON.stringify([entry.personaId || "", entry.persona || "", entry.kind || "", entry.body || ""]);
  ...
  if (extracted && entry.body && dlExplicit(entry.body.split("\n")[0]) !== entry.date) {
    ...
    if (entry.body.replace(/\r\n?/g, "\n").trim() === withoutDate) entry.body = extracted.body;
  }
  return before !== JSON.stringify([entry.personaId || "", entry.persona || "", entry.kind || "", entry.body || ""]);
}
function dlList() {
...
function dlEntryPersona(entry) {
  return personaDisplay(entry?.personaId || entry?.persona || "ペルソナ未設定");
}
function dlSortOrder() {
  return settings.diarySortOrder === "asc" ? "asc" : "desc";
}
function dlCompareDate(a, b) {
  const compared = (a.date || "").localeCompare(b.date || "");
  return dlSortOrder() === "asc" ? compared : -compared;
}
function dlTodayJst() {
  return new Intl.DateTimeFormat("sv-SE", { timeZone: "Asia/Tokyo", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date());
}
function dlMissingDates(month, entries) {
  if (!/^\d{4}-\d{2}$/.test(month)) return [];
  const today = dlTodayJst();
  if (month > today.slice(0, 7)) return [];
  const [year, monthNumber] = month.split("-").map(Number);
  const lastDay = new Date(Date.UTC(year, monthNumber, 0)).getUTCDate();
  const limit = month === today.slice(0, 7) ? Math.min(lastDay, Number(today.slice(8, 10))) : lastDay;
...
function dlMonthTimeline(entries, missing) {
  return entries.map((entry, index) => ({ date: entry.date || "", index, entry }))
    .concat(missing.map((date, index) => ({ date, index, missing: true })))
    .sort((a, b) => {
      const compared = a.date.localeCompare(b.date);
      if (compared) return dlSortOrder() === "asc" ? compared : -compared;
...
async function dlExcludeCandidate(candidate) {
  ...
  if (!dlExcluded().some(x => dlSourceKey(x) === dlSourceKey(candidate))) {
    dlExcluded().push({ sessionId: candidate.sessionId, messageId: candidate.messageId, date: candidate.date, persona: candidate.persona, personaId: candidate.personaId || personaIdFor(candidate.persona), kind: candidate.kind, preview: candidate.body.slice(0, 160), excludedAt: Date.now() });
  }
...
function dlMarkedDiary(text) {
  ...
  for (let index = 0; index < lines.length; index++) {
    const header = lines[index].replace(/^\s*```(?:text|markdown)?\s*/i, "").trim();
    const match = header.match(/^📓\s*(.+?)\s+(20\d{2}[-\/年]\d{1,2}[-\/月]\d{1,2}日?)\s*$/);
    if (!match) continue;
    const profile = personaProfileForEmoji(match[1]);
    if (!profile) continue;
    const body = [];
    for (let bodyIndex = index + 1; bodyIndex < lines.length; bodyIndex++) {
      const line = lines[bodyIndex];
      if (/^\s*---\s*$/.test(line) || /^\s*📓/.test(line) || /^\s*```\s*$/.test(line)) break;
      body.push(line);
    }
    const cleanBody = body.join("\n").trim();
    if (!cleanBody) continue;
    const date = dlExplicit(match[2]);
    return {
      date,
      body: date + "\n\n" + cleanBody,
...
function dlLegacyDelimitedDiary(text) {
  const lines = String(text || "").replace(/\r\n?/g, "\n").split("\n");
  for (let index = 0; index < lines.length; index++) {
    const header = lines[index].replace(/^\s*```(?:text|markdown)?\s*/i, "").trim();
    if (!/^20\d{2}[-\/年]\d{1,2}[-\/月]\d{1,2}日?$/.test(header)) continue;
    const end = lines.findIndex((line, lineIndex) => lineIndex > index && /^\s*---\s*$/.test(line));
    if (end < 0) continue;
    const content = lines.slice(index + 1, end).join("\n").trim();
    if (!content) continue;
    const date = dlExplicit(header);
    return { date, body: date + "\n\n" + content };
  }
  return null;
}
function dlKindMention(text) {
  const raw = String(text || "");
  if (/締めログ/.test(raw)) return "closing";
  if (/#今日の記録|今日の記録/.test(raw)) return "today";
  const kinds = [...new Set(personaProfiles().filter(profile => profile.diaryLabel && raw.includes(profile.diaryLabel)).map(profile => profile.diaryKind))];
...
function dlBody(text, kind) {
  ...
  const block = blocks.find(x => dlExplicit(x) || dlKindMention(x));
  if (block) return block;
  const dateStart = raw.search(/20\d{2}[-\/年]\d{1,2}[-\/月]\d{1,2}日?/);
  const labels = [...new Set(personaProfiles().filter(profile => profile.diaryKind === kind).map(profile => profile.diaryLabel).filter(Boolean))];
  if (kind === "closing" && !labels.includes("締めログ")) labels.push("締めログ");
  const labelStarts = labels.map(label => raw.indexOf(label)).filter(index => index >= 0);
  const start = dateStart >= 0 ? dateStart : labelStarts.length ? Math.min(...labelStarts) : -1;
...
function dlScan() {
  ...
  activeSessions().forEach(s => (s.messages || []).forEach((m, i) => {
    ...
    const marked = dlMarkedDiary(answer);
    const legacyDelimited = marked ? null : dlLegacyDelimitedDiary(answer);
    const answerKind = dlKindMention(answer);
    const requestedKind = dlKindMention(ask);
...
function dlShowDuplicate(candidate) {
  ...
  const matches = dlMatchingEntries(candidate);
  if (!matches.length) {
    alert("登録済みの日記が見つかりませんでした。日記一覧を更新して、もう一度お試しください。");
    return;
  }
  const d = document.createElement("dialog");
  d.id = "dlDuplicateViewer";
  d.className = "dl-dialog";
  d.innerHTML = '<form method="dialog"><div class="dialog-head"><div><p class="browser-kicker">REGISTERED DIARY</p><h2>登録済みの日記</h2></div><button value="cancel" class="icon-btn">×</button></div><p class="muted">候補と同じ日付・種別・ペルソナで登録されている日記です。</p><div class="dl-list">' + matches.map(entry => '<article class="dl-card"><span class="dl-kind">' + esc(diaryKindLabel(entry.kind, entry.personaId || entry.persona)) + '</span><h3>' + esc(entry.date) + '</h3><small>' + esc(dlEntryPersona(entry)) + (entry.model ? " · " + esc(entry.model) : "") + '</small><div class="dl-body">' + esc(entry.body) + '</div><div class="dl-actions"><button type="button" data-dldup-edit="' + esc(entry.id) + '">この日記を修正</button></div></article>').join("") + '</div><div class="dialog-actions"><button value="cancel">閉じる</button></div></form>';
  document.body.append(d);
  d.addEventListener("close", () => d.remove(), { once: true });
  d.querySelectorAll("[data-dldup-edit]").forEach(button => button.onclick = () => {
    const entry = dlList().find(item => item.id === button.dataset.dldupEdit);
    d.close();
    if (entry) dlEditor(entry);
  });
  d.showModal();
}
...
function dlPersonaSelectOptions(base) {
  ...
  const options = [...new Set([...personaOptions(), ...(current && current !== "未分類" ? [current] : [])])];
  return '<option value="" ' + (!current ? "selected" : "") + '>ペルソナ未設定</option>' + options.map(name =>
    '<option value="' + esc(name) + '" ' + (name === current ? "selected" : "") + '>' + esc(name) + '</option>'
  ).join("");
}
function dlKindSelectOptions(base) {
  const currentKind = normalizeDiaryKind(base?.kind, base?.personaId || personaIdFor(base?.persona));
  const rows = personaProfiles().map(profile => ({ kind: profile.diaryKind, label: profile.diaryLabel }));
  if (!rows.some(row => row.kind === currentKind)) rows.push({ kind: currentKind, label: diaryKindLabel(currentKind, base?.personaId || base?.persona) });
  const unique = new Map(rows.map(row => [row.kind, row]));
  return Array.from(unique.values()).map(row => '<option value="' + esc(row.kind) + '">' + esc(row.label) + '</option>').join("");
}
function dlSyncKindToPersona(dialog) {
  const persona = dialog.querySelector("#dlPersona").value;
  const select = dialog.querySelector("#dlKind");
  const profile = personaProfileFor(persona);
  if (profile) select.value = profile.diaryKind;
  select.disabled = Boolean(profile);
  select.title = profile ? "ペルソナ管理小屋の設定に連動しています" : "";
}
...
function dlSelected() {
  ...
  const kind = dlKindMention(text) || diaryKindForPersona(persona);
...
function dlCard(entry) {
  return '<article class="dl-card"><span class="dl-kind">' + esc(diaryKindLabel(entry.kind, entry.personaId || entry.persona)) + '</span><h3>' + esc(entry.date) + '</h3><small>' + esc(entry.model || "モデル未設定") + '</small><div class="dl-body">' + esc(entry.body) + '</div><div class="dl-actions">' + (entry.sessionId ? '<button class="dl-jump" data-dlj="' + esc(entry.id) + '">元の位置で見る</button>' : "") + '<button data-dle="' + esc(entry.id) + '">内容修正</button><button data-dld="' + esc(entry.id) + '">削除</button></div></article>';
}
function dlPayload(entries, persona, month) {
  return {
    ...
    entries: entries.map(entry => ({
      date: entry.date,
      kind: diaryKindLabel(entry.kind, entry.personaId || entry.persona),
...
function dlMarkdown(entries, persona, month) {
  ...
  return "# " + persona + " " + label + "\n\n" + entries.map(entry =>
    "## " + entry.date + "｜" + diaryKindLabel(entry.kind, entry.personaId || entry.persona) +
    (entry.model ? "\n\nモデル：" + entry.model : "") + "\n\n" + entry.body
  ).join("\n\n---\n\n") + "\n";
}
...
function dlGroupedRows(rows) {
  ...
  return Array.from(personas.entries()).map(([persona, months]) => {
    ...
    const monthHtml = Array.from(months.entries()).sort((a, b) => dlSortOrder() === "asc" ? a[0].localeCompare(b[0]) : b[0].localeCompare(a[0])).map(([month, entries], monthIndex) => {
      const label = month === "日付不明" ? month : month.slice(0, 4) + "年" + Number(month.slice(5, 7)) + "月";
...
function dlRender() {
  ...
  if ($("dlOpenPersonaHut")) $("dlOpenPersonaHut").onclick = openPersonaHut;
  document.querySelectorAll("[data-dl-template]").forEach(button => button.onclick = async () => {
    const profile = personaProfiles().find(item => item.id === button.dataset.dlTemplate);
    if (!profile) return;
    await copyToClipboard(dlTemplateText(profile));
    const previous = button.textContent;
    button.textContent = "コピーしました";
    setTimeout(() => { if (button.isConnected) button.textContent = previous; }, 1400);
  });
  if ($("dlSort")) {
    $("dlSort").value = dlSortOrder();
    $("dlSort").onchange = async event => {
      settings.diarySortOrder = event.target.value === "asc" ? "asc" : "desc";
      await save();
      dlRender();
    };
  }
